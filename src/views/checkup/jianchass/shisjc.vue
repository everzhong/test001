  ...
  <div class="app-container container_1">
    <section v-show="!heshiOption.show">
          ...
          >
            <el-form-item label="案件来源" prop="ajly">
              <el-input
                readonly
                :value="
                  selectDictLabels($store.getters.ajlyDic, queryInfoFrom.ajly)
                "
              ></el-input>
            </el-form-item>
            <el-form-item label="险种" prop="ybbf">
              <el-input
                readonly
                :value="
                  selectDictLabels($store.getters.ybbfDic, queryInfoFrom.ybbf)
                "
              ></el-input>
            </el-form-item>
            <el-form-item label="就医类型" prop="jslb">
              <el-input
                readonly
                :value="
                  selectDictLabels($store.getters.jslbDic, queryInfoFrom.jslb)
                "
              ></el-input>
            </el-form-item>
            ...
            <el-form-item label="数据开始日期" prop="datastarttime">
              <el-input
                readonly
                v-model="queryInfoFrom.datastarttime"
              ></el-input>
            </el-form-item>
            ...
            >
              ...
              <el-button
                type="primary"
                size="mini"
                @click="heshiOption.show = true"
                >机构核实</el-button
              >
              <el-button type="primary" size="mini" @click="doSubmit"
                >检查完成</el-button
              >
      ...
      <div class="table-main" :style="{ top: topValue }">
        <el-row :gutter="10" class="mb5">
          ...
          <el-col
            ...
            v-if="!queryInfoFrom.fromLuli && tabsValue === 'three'"
          >
            <el-button type="primary" size="small" @click="handleThirdCheck"
              >开展第三方筛查</el-button
            >
          </el-col>
          <el-col
          ...
          <el-col
            :span="1.5"
            v-if="!queryInfoFrom.fromLuli && tabsValue === 'six'"
          >
            <el-button type="primary" size="small" @click="showHecha = true"
              >选择核查数据</el-button
            >
          </el-col>
          <el-col
          ...
          <div
            class="top-right-btn"
            v-if="tabsValue !== 'three' && tabsValue !== 'six'"
          >
            <el-col :span="1.5">
              <el-button type="default" size="small" @click="goBackUpLevel"
                >返回上一层</el-button
              >
            </el-col>
          </div>
          ...
          <div
            class="top-right-btn"
            v-if="tabsValue == 'five' || tabsValue == 'qmx'"
          >
            <el-radio-group
              v-model="tabsValue"
              size="small"
              @change="tabsLevelChange"
            >
        ...
        <div
          v-loading="loading"
          v-if="tabsValue !== 'four' && tabsValue !== 'six'"
          style="height: calc(100% - 37px)"
        >
          <sTable
            v-if="tabsValue == 'three'"
            :data="renwuthreeList"
            :header="tableHeader"
            :fixedNum="2"
            :checkAll="false"
            @selection-change="mutilLiushuihz"
          >
            <el-table-column
              label="序号"
              width="55"
          ...
          <tongliumx
            ref="tongLiumx"
            v-if="
              (tabsValue === 'five' || tabsValue === 'qmx') && !qmxOptions.show
            "
        ...
        <div
          ...
          v-else
          :style="{
            height: `calc(100% - ${tabsValue == 'six' ? 153 : 113}px)`,
          }"
        >
          <jinxiaohecha
            v-show="tabsValue == 'six'"
            :tableData="renwusixList"
          ...
          <liushui-table
            v-if="tabsValue == 'four'"
            ref="liuShuiTable"
            :fromLog="queryInfoFrom.fromLuli"
            :tableData="renwufourList"
            @radio-change="handleSelectionChange"
            @checkdetail="tongLiushuimx"
            @on-log="checkLog"
          ></liushui-table>
        </div>
      </div>
      <div
        class="fixed-bottom"
        :class="[
          tabsValue == 'four' || tabsValue == 'six' ? 'fixed-bottom1' : '',
        ]"
      >
        <pagination
        ...
        <div
          v-show="
            (tabsValue == 'four' || tabsValue == 'six') &&
            !logShow &&
            !queryInfoFrom.fromLuli &&
            !qmxOptions.show
          "
          class="xingweirz"
        >
          ...
          >
            ...
            >
              <el-input
                ...
                v-model="xwrdForm.zkdj"
                :disabled="!!isDisabled.dj"
                @change="handleDjslChange"
              ></el-input>
            </el-form-item>
            <el-form-item
              label="追款单价"
            ...
            >
              <el-input
                onkeyup="this.value=this.value.replace(/[^0-9.]/g,'')"
                onafterpaste="this.value=this.value.replace(/[^0-9.]/g,'')"
                style="width: 90%; margin-right: 3px"
                v-model="xwrdForm.wgsl"
                :disabled="!!isDisabled.sl"
                @change="handleDjslChange"
              ></el-input>
              <el-tooltip
                v-if="tabsValue === 'six'"
                class="item"
                effect="dark"
                content="默认为差额数量，差额数量=医保结算数量-(期初库存数量+本期购入数据-期末库存数量-现金销售数量)"
                placement="top"
              >
                <i
            ...
            >
              <el-input
                type="number"
                min="0"
                v-model="xwrdForm.qckc"
                :disabled="!pksj"
                @change="hanndelChange"
            ...
            <el-form-item>
              <el-button type="primary" size="mini" @click="xwrdSubmit"
                >确定</el-button
              >
            </el-form-item>
          </el-form>
          ...
          <el-form-item label="涉及结算人次数" prop="xjjzrs">
            <div class="item-group">
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.xjjzrs"
              ></el-input>
              <span>-</span>
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.sjrcs"
              ></el-input>
            </div>
          </el-form-item>
          <el-form-item label="涉及项目数" prop="xjmxs">
            <div class="item-group">
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.xjmxs"
              ></el-input>
              <span>-</span>
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.bz"
              ></el-input>
            </div>
          </el-form-item>
          <el-form-item label="涉及金额" prop="xjje">
            <div class="item-group">
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.xjje"
              ></el-input>
              <span>-</span>
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.ydsm"
              ></el-input>
            </div>
          </el-form-item>
          <el-form-item label="结算总费用" prop="jsfy">
            <div class="item-group">
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.jsfy"
              ></el-input>
              <span>-</span>
              <el-input
                type="number"
                min="0"
                v-model="gzQueryForm.jsdj"
              ></el-input>
            </div>
          </el-form-item>
        </div>
    ...
    <xwrd-dialog
      ...
      v-if="xwrdDialog.show"
    ></xwrd-dialog>
    <jgheshi
      :options="heshiOption"
      v-if="heshiOption.show"
...
export default {
  ...
  methods: {
    ...
    tabsLevelChange(val) {
      ...
      this.searchTlsNextParams =
        val == "five"
          ? { lsh: this.lsh || "", mxxmbm: this.mxxmbm }
          : {
              jgdm: jgdm,
        ...
        .then(() => {
          ...
          setShujusc({
            ...
            .then((res) => {
              ...
              this.$router.push({
                path: "/zhgl/dsfgz/fasc/scenarioConfiguration",
              });
            })
            .catch(() => {
              this.loading = false;
    ...
    xwrdSubmit() {
      ...
      if (this.tabsValue === "six" && !this.pksj) {
        //进销存核查
    ...
    //批量产线流水号项目汇总
    mutilLiushuihz(selection) {
      if (selection.length !== 0) {
        this.mutilSelect = [...selection];
      }
    },
    //同流水明细
    tongLiushuimx(row) {
    ...
    handleDjslChange() {
      if (this.xwrdForm.zkdj !== "" && this.xwrdForm.wgsl !== "") {
        this.xwrdForm.wgfy = (this.xwrdForm.zkdj * this.xwrdForm.wgsl).toFixed(
          2
        );
      }
    },
